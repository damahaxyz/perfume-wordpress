...
plugin/perfumehouse-plugin/
├── perfumehouse-plugin.php        插件入口与常量
...
- 主题函数前缀：`perfumehouse_`
- 插件命名空间：`PerfumeHouse\Plugin`
- 插件常量前缀：`PERFUMEHOUSE_`
- 主题 text domain：`perfumehouse`
- 插件 text domain：`perfumehouse-plugin`

## 本地检查

```bash
find theme/perfumehouse plugin/perfumehouse-plugin \
...
脚本默认部署到 `root@perfumehouse:/root/perfumehouse-shop`，并检查
`https://www.perfumehouse.vip/`。
...
DEPLOY_PATH=/opt/wordpress-sites/perfumehouse-shop \
