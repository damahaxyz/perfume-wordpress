...
deploy_host="${DEPLOY_HOST:-root@perfumehouse}"
deploy_path="${DEPLOY_PATH:-/root/perfumehouse-shop}"
deploy_url="${DEPLOY_URL:-https://www.perfumehouse.vip/}"
...
for component in "$@"; do
    case "${component}" in
        theme)
            ...
            component_slugs+=("perfumehouse")
            local_paths+=("${project_dir}/theme/perfumehouse")
            required_files+=("style.css")
            ;;
        plugin)
            component_types+=("plugin")
            component_slugs+=("perfumehouse-plugin")
            local_paths+=("${project_dir}/plugin/perfumehouse-plugin")
            required_files+=("perfumehouse-plugin.php")
            ;;
        *)
